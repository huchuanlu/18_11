...
// 	int *ir, *jc;
  ...
  //   printf("Line 68");
    ...
    npp_me = mxGetM(in[3]);
    nsp = npp_me-1;
   if (mxGetM(in[3]) * mxGetN(in[3]) != npp_me) {
 ...
 //   printf("Line 75");
  ...
  //   printf("Line 81");
    /* create output */
    out[0] = mxCreateSparse(nsp,nsp,pj[nsp],mxREAL);
    if (out[0]==NULL) {
...
//     printf("Line 91");
 ...
 //    printf("Line 107");
	...
	//printf("%d / %d\n",j,np);
        jc[j] = total;
        jx = sp_ind_middle[j] / nr; /* col */
        jy = sp_ind_middle[j] % nr; /* row */
//      printf("Line116");   
        for (k=pj[j]; k<pj[j+1]; k++) {
         ...
         printf("Line 125\n");
                ix = sp_ind_middle[i] / nr; 
                iy = sp_ind_middle[i] % nr;
